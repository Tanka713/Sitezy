...
Return JSON:
{
  ...
  "designDirection": "string (2-3 sentences: overall mood, component style, typography use — be specific and industry-appropriate)",
...
- Do NOT use negative top margins, negative translateY offsets, or absolute positioning that pulls the first section up underneath the navbar
${logoInstruction}
...
${instruction ? `\nSPECIAL DIRECTION FOR THIS REGENERATION: ${instruction}` : ""}`.trimEnd();
...
${instruction ? `Additional direction: ${instruction}` : ""}`;
  ...
  const html = await streamCompletion(system, user, () => {}, 8000);

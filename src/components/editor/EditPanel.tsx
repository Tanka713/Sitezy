...
const PANEL_CARD = "rounded-[24px] border border-white/[0.08] bg-[linear-gradient(180deg,rgba(255,255,255,0.04),rgba(255,255,255,0.015))] shadow-[0_18px_40px_rgba(0,0,0,0.18)]";
const PANEL_SUBCARD = "rounded-[18px] border border-white/[0.08] bg-white/[0.03] px-3 py-2.5";
const PANEL_INPUT =
  "w-full min-w-0 rounded-[18px] border border-white/[0.08] bg-white/[0.045] px-3 py-2 text-[11px] text-white/80 placeholder-white/20 shadow-[inset_0_1px_0_rgba(255,255,255,0.03)] focus:outline-none focus:border-teal-300/30 focus:bg-white/[0.06] transition-all";
const PANEL_SELECT = `${PANEL_INPUT} appearance-none`;
const PANEL_BUTTON_PRIMARY =
  "px-3 py-2 rounded-[18px] border border-teal-300/18 bg-teal-400/10 text-[10px] font-semibold text-teal-100 hover:bg-teal-400/16 transition-all";
const PANEL_BUTTON_SECONDARY =
  "px-2.5 py-2 rounded-[18px] border border-white/[0.08] bg-white/[0.04] text-[10px] font-semibold text-white/48 hover:text-white/78 hover:border-white/[0.16] transition-all";
...
function ColorPicker({ value, onChange, onClear, allowClear = false }: {
  ...
  return (
    <div className="relative" ref={pickerRef}>
      ...
      {open && (
        <div className="mt-1.5 w-full overflow-hidden rounded-[18px] border border-white/[0.09] bg-[#14191f] shadow-2xl">
          {/* Gradient square */}
          <div
            ref={gradRef}
...
function Accordion({ title, icon, open, toggle, children }: {
  ...
  return (
    <div className={`${PANEL_CARD} overflow-hidden ${open ? "border-white/[0.11]" : ""}`}>
      <button onClick={toggle}
        className="w-full flex items-center gap-3 px-4 py-3.5 text-left hover:bg-white/[0.035] transition-colors group">
        <span className={`flex h-7 w-7 items-center justify-center rounded-lg border transition-colors ${
          open
            ? "border-teal-300/18 bg-teal-400/10 text-teal-100"
            : "border-white/[0.07] bg-white/[0.03] text-white/28 group-hover:text-white/52"
        }`}>
          {icon}
        </span>
        <span className="flex-1 text-[10px] font-bold text-white/58 uppercase tracking-[0.18em] group-hover:text-white/76 transition-colors">
          {title}
        </span>
        <ChevronDown size={11} className={`text-white/24 transition-transform flex-shrink-0 ${open ? "rotate-180 text-white/55" : ""}`} />
      </button>
      {open && <div className="px-4 pb-4 pt-0 space-y-3.5">{children}</div>}
    </div>
  );
}
    ...
    <div className="flex items-center gap-1 rounded-xl border border-white/[0.08] bg-white/[0.04] p-1">
      {options.map((o) => (
        <button key={o.val} title={o.title} onClick={() => onChange(o.val)}
          className={`flex-1 h-8 flex items-center justify-center rounded-lg border transition-all text-[11px] ${
            value === o.val
              ? "border-teal-300/18 bg-teal-400/12 text-white"
...
function PresetSelect({
  ...
  return (
    <select
      ...
      className="h-10 min-w-[96px] rounded-xl border border-white/[0.08] bg-white/[0.045] px-3 text-[11px] text-white/72 focus:outline-none appearance-none leading-none"
    >
  ...
  return (
    <div className="space-y-2.5">
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="h-10 w-[96px] flex-shrink-0 rounded-xl border border-white/[0.08] bg-white/[0.045] px-2.5 shadow-[inset_0_1px_0_rgba(255,255,255,0.03)] focus-within:border-indigo-400/35 focus-within:bg-white/[0.06] transition-all">
          <div className="flex h-full items-center gap-1.5">
            <input
              ...
              className="editor-plain-input h-full w-full bg-transparent text-[11px] text-white/74 focus:outline-none [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none"
            />
            {unit ? (
...
export function EditPanel({ iframeRef, onClose, project }: Props) {
  ...
  const [open, setOpen] = useState<Record<string, boolean>>({
    ...
    spacing: true, background: false, border: false, effects: false, animation: false, section: false, icon: false,
  });
  ...
  useEffect(() => {
    ...
    if ((node as any).isIframe) {
      ...
      if (activeField !== "embedUrl") setEmbedUrl(nextEmbedUrl);
      setEmbedMode(inferEmbedMode(nextEmbedUrl));
    } else if (activeField !== "embedUrl") {
      setEmbedUrl("");
      setEmbedMode("custom");
    }
    if (activeField !== "linkUrl") {
    ...
    if (activeField !== "top") setTopVal(node.top || "auto");
    if (activeField !== "right") setRightVal(node.right || "auto");
    if (activeField !== "bottom") setBottomVal(node.bottom || "auto");
    if (activeField !== "left") setLeftVal(node.left || "auto");
    if (activeField !== "zIndex") setZIndexVal(node.zIndex || "auto");
    if (activeField !== "flexGrow") setFlexGrowVal(parseSizeToken(node.flexGrow, 0));
    if (activeField !== "flexShrink") setFlexShrinkVal(parseSizeToken(node.flexShrink, 1));
    if (activeField !== "flexBasis") setFlexBasisVal(node.flexBasis || "auto");
    if (activeField !== "gap") setGapVal(parseSizeToken(node.gap, 0));
    if (activeField !== "rowGap") setRowGapVal(parseSizeToken(node.rowGap || node.gap, 0));
    if (activeField !== "columnGap") setColumnGapVal(parseSizeToken(node.columnGap || node.gap, 0));
    if (activeField !== "gridCols") setGridColsVal(node.gridCols || "");
    if (activeField !== "gridRows") setGridRowsVal(node.gridRows || "");
    if (activeField !== "blur") setBlurVal(parseFilterFn(node.filter, "blur", 0));
    if (activeField !== "brightness") setBrightnessVal(parseFilterFn(node.filter, "brightness", 100));
    if (activeField !== "contrast") setContrastVal(parseFilterFn(node.filter, "contrast", 100));
    if (activeField !== "grayscale") setGrayscaleVal(parseFilterFn(node.filter, "grayscale", 0));
    if (activeField !== "saturate") setSaturateVal(parseFilterFn(node.filter, "saturate", 100));
    if (activeField !== "backdropBlur") setBackdropBlurVal(parseFilterFn(node.backdropFilter, "blur", 0));
    if (activeField !== "mixBlend") setMixBlendVal(node.mixBlendMode || "normal");
    if (activeField !== "animDuration") setAnimDurationVal(parseTimeToMs(node.animationDuration, 600));
    if (activeField !== "animDelay") setAnimDelayVal(parseTimeToMs(node.animationDelay, 0));
    ...
    if (activeField !== "borderWidth") setBorderWidthVal(parseSizeToken(node.borderWidth, 0));
    if (activeField !== "borderStyle") setBorderStyleVal(node.borderStyle || "none");
    if (activeField !== "borderColor") setBorderColorVal(safeHex(node.borderColor ?? node.color));
    ...
    setOpen((o) => ({
      ...
      link: !!(node.isText || node.isBtn || node.tag === "a" || node.href !== null),
      section: !!node.isSec || (!node.isSec && (!!node.secHasBgImage || !!node.sectionVisualNodeId)),
      animation: (node.animationIn && node.animationIn !== "none") || (node.animationHover && node.animationHover !== "none") || o.animation,
      icon: !!(node as any).isSvg,
    }));
  ...
  const clearResponsiveOverrides = useCallback(() => {
    ...
    const nodeIds = Array.from(
      new Set(
        [node.nodeId, node.mediaTargetNodeId, node.animationTargetNodeId].filter(
  ...
  const resetResponsiveProp = useCallback((props: string[], scope: "node" | "media" | "all" = "node") => {
    if (!node || devicePreview === "desktop") return;
    const targets =
      scope === "media"
        ? [node.mediaTargetNodeId ?? node.nodeId]
        : scope === "all"
          ? [node.nodeId, node.mediaTargetNodeId, node.animationTargetNodeId]
          : [node.nodeId];
  ...
  if (!node) {
    return (
      <div className="editor-inspector flex h-full flex-col items-center justify-center gap-4 px-6 text-center">
        <div className="flex h-14 w-14 items-center justify-center rounded-[22px] border border-white/[0.08] bg-[linear-gradient(180deg,rgba(45,212,191,0.18),rgba(251,191,36,0.06))] shadow-[0_18px_36px_rgba(0,0,0,0.2)]">
          <MousePointer2 size={18} className="text-teal-100/84" />
        </div>
        <div className="space-y-1.5 max-w-[240px]">
          <p className="text-[12px] font-semibold text-white/82">Select something to style</p>
          <p className="text-[11px] text-white/34 leading-relaxed">
            Click any element on the canvas to open its design controls, responsive overrides, and element settings.
          </p>
        </div>
      </div>
    );
  ...
  const showLayoutPanel = node.isSec || node.isContainer || isMediaNode || node.isBtn || node.isInput;
  // Sections already have their own background picker inside the "Section" accordion — don't duplicate it
  const showBackgroundPanel = !node.isSec && (node.isContainer || isMediaNode || node.isBtn || node.isInput || node.hasBgImage);
  const showBorderPanel = node.isSec || node.isContainer || isMediaNode || node.isBtn || node.isInput;
  const showLinkPanel = node.isText || node.isBtn || node.tag === "a" || node.href !== null;
  ...
  return (
    <div className="editor-inspector flex flex-col h-full overflow-hidden bg-[linear-gradient(180deg,rgba(255,255,255,0.02),rgba(255,255,255,0))]">
      <div className="px-3 py-3.5 border-b border-white/[0.06] flex-shrink-0 bg-[linear-gradient(180deg,rgba(255,255,255,0.035),rgba(255,255,255,0.008))]">
        <div className={`${PANEL_CARD} px-3.5 py-3.5`}>
          <div className="flex items-center gap-2.5 min-w-0">
            <span className="rounded-lg border border-teal-300/18 bg-teal-400/10 px-2 py-1 text-[10px] font-mono font-semibold text-teal-100 flex-shrink-0">
              {node.tag}
            </span>
            {node.label && (
              <span className="text-[11px] text-white/72 truncate font-semibold">{node.label}</span>
            )}
            {node.isSec && (
              <span className="ml-auto rounded-lg border border-amber-200/16 bg-amber-200/10 px-2 py-1 text-[9px] font-semibold text-amber-100/84 flex-shrink-0">
                section
              </span>
            )}
          </div>
          {node.sectionName && (
            <p className="text-[9.5px] text-white/26 truncate font-mono mt-2">
              {node.sectionName} › {node.role}
            </p>
          )}
          {!node.sectionName && (
            <p className="text-[9.5px] text-white/24 truncate font-mono mt-2">
              {node.role}
            </p>
          )}
          <div className="mt-2 flex flex-wrap gap-1.5">
            <div className="inline-flex items-center gap-1 rounded-full border border-white/[0.08] bg-white/[0.03] px-2 py-1 text-[9px] font-semibold uppercase tracking-[0.12em] text-white/42">
              Inspector
            </div>
            {devicePreview !== "desktop" && (
              <div className="inline-flex items-center gap-1 rounded-full border border-sky-400/20 bg-sky-400/10 px-2 py-1 text-[9px] font-semibold uppercase tracking-[0.12em] text-sky-200/80">
                {devicePreview} styles
              </div>
            )}
            {editing && (
              <div className="inline-flex items-center gap-1 rounded-full border border-emerald-400/18 bg-emerald-400/10 px-2 py-1 text-[9px] font-semibold uppercase tracking-[0.12em] text-emerald-200/78">
                live editing
              </div>
            )}
          </div>
      ...
      <div className="flex-1 overflow-y-auto px-2.5 py-3 space-y-3">
                  ...
                  <button
                    ...
                    className={`rounded-xl border py-2 text-[10px] font-semibold uppercase tracking-[0.14em] transition-all ${
                      devicePreview === device
                        ? "border-indigo-400/28 bg-indigo-500/12 text-indigo-200"
                        : "border-white/[0.08] bg-white/[0.03] text-white/54 hover:border-white/[0.16] hover:text-white/80"
                    }`}
                  >
                    Only {device}
        ...
        {showLinkPanel && (
          <Accordion title="Link" icon={<Link size={12}/>} open={open.link} toggle={() => toggle("link")}>
            {node.isText && node.tag !== "a" && !node.href && (
              <p className="text-[10px] text-white/28 leading-relaxed">
                Choose a link type below to turn this text into a clickable hyperlink.
              </p>
            )}
            {/* Type selector */}
                  ...
                  return (
                    <button key={p.id} type="button"
                      ...
                      className={`flex items-center gap-2 w-full px-2 py-1.5 rounded-md text-[11px] transition-all text-left ${
                        isSel ? "bg-indigo-500/15 text-indigo-300" : "text-white/45 hover:bg-white/[0.04] hover:text-white/75"
                      }`}>
                      <FileText size={9} className="flex-shrink-0 opacity-50" />
                      <span className="truncate">{p.name}</span>
                      {isSel && <span className="ml-auto text-[9px] text-indigo-400/60 flex-shrink-0">selected</span>}
                    </button>
                  );
                })}
              </div>
            ...
            {linkType === "page" && (() => {
              ...
              return targetPage ? (
                <button onClick={() => selectPage(targetPage.id)}
                  className="w-full flex items-center justify-center gap-1.5 py-1.5 rounded-lg text-[10px] font-medium border border-indigo-500/20 text-indigo-400 hover:bg-indigo-500/10 transition-all">
            ...
            {(linkType === "url" || linkType === "page") && (
              <label className="flex items-center justify-between cursor-pointer">
                ...
                <button
                  ...
                  data-active={node.target === "_blank"}
                  data-size="sm"
                  className="editor-switch"
                >
                  <span className="editor-switch-thumb" />
        ...
        {/* ── Typography ── */}
        {(node.isText || node.isBtn || node.tag === "a" || (node as any).isInput) && (
          <Accordion title="Typography" icon={<Type size={12}/>} open={open.typography} toggle={() => toggle("typography")}>
            ...
            <div>
              ...
              <ColorPicker value={txtColor} onChange={(hex) => applyStyle("color", hex, { batch: true })} />
            </div>
              ...
              <div className="flex items-center justify-between mb-1.5">
                ...
                <button onClick={() => {
                  ...
                  if (next) {
                    ...
                    send("apply-style", { prop: "backgroundImage", value: grad });
                    send("apply-style", { prop: "webkitBackgroundClip", value: "text" });
                    send("apply-style", { prop: "backgroundClip", value: "text" });
                    send("apply-style", { prop: "webkitTextFillColor", value: "transparent" });
                  } else {
                    send("apply-style", { prop: "backgroundImage", value: "none" });
                    send("apply-style", { prop: "webkitBackgroundClip", value: "" });
                    send("apply-style", { prop: "backgroundClip", value: "" });
                    send("apply-style", { prop: "webkitTextFillColor", value: "" });
                  }
                }}
                data-active={gradientOn}
                data-size="sm"
                className="editor-switch"
              >
                  <span className="editor-switch-thumb pointer-events-none" />
                </button>
              </div>
              {gradientOn && (
                <div className="space-y-2">
                  <div className="grid grid-cols-3 gap-1">
                    {GRADIENT_DIRS.map(d => (
                      <button key={d.value} onClick={() => {
                        ...
                        send("apply-style", { prop: "backgroundImage", value: grad });
                      }} className={`py-1 rounded-lg text-[11px] border transition-all ${gradDir === d.value ? "border-indigo-500/40 bg-indigo-500/10 text-indigo-300" : "border-white/[0.06] text-white/50 hover:border-white/20"}`}>
                        {d.label}
                      </button>
                    ))}
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      ...
                      <ColorPicker value={gradFrom} onChange={(hex) => {
                        ...
                        send("apply-style", { prop: "backgroundImage", value: grad, batch: true });
                      }} />
                    </div>
                    <div>
                      <Label>To</Label>
          ...
          <Accordion title="Icon" icon={<Layers size={12}/>} open={open.icon} toggle={() => toggle("icon")}>
            ...
            <div className="relative">
              <input
                ...
                className="w-full bg-white/[0.05] border border-white/[0.07] rounded-lg px-2.5 py-1.5 text-[11px] text-white/60 placeholder-white/20 focus:outline-none focus:border-indigo-500/35 transition-colors"
              />
            </div>
                ...
                .map((ic: IconDef) => {
                  ...
                  return (
                    <button
                      ...
                      className="flex items-center justify-center w-full aspect-square rounded-lg bg-white/[0.03] border border-white/[0.05] hover:bg-indigo-500/10 hover:border-indigo-500/25 transition-all group"
                    >
                      <svg
                        viewBox="0 0 24 24"
                        width="16"

...
export function PreviewCanvas({ project, iframeRef }: Props) {
  ...
  // Force iframe rewrite when switching back to preview/split from code
  ...
  useEffect(() => {
    function handler(e: MessageEvent) {
      ...
      if (type === "toggle-edit-mode") {
        setVisualEditMode(!visualEditModeRef.current);
      }
      if (type === "hover") {
  ...
  }, [visualEditMode, previewMode, isIframeEditing]);
  ...
  return (
    <div className="editor-workspace flex-1 flex min-w-0 flex-col overflow-hidden">

      <div className="border-b border-white/[0.06] bg-[linear-gradient(180deg,rgba(255,255,255,0.035),rgba(255,255,255,0.012))] px-3 py-2 flex-shrink-0">
        <div className="flex flex-wrap items-center gap-2">
          <div className="mr-2 min-w-0">
            <p className="text-[10px] font-semibold uppercase tracking-[0.22em] text-white/26">Canvas</p>
            <p className="mt-0.5 truncate text-[12px] font-medium text-white/68">{page?.name ?? "Select a page to start editing"}</p>
          </div>

          <div className="editor-panel-soft flex items-center gap-1 rounded-[18px] p-1">
          {([
            ...
            <button key={mode}
              ...
              title={label}
              className={`flex h-7 items-center gap-1 rounded-xl px-2.5 text-[11px] transition-colors ${previewMode === mode ? "border border-white/[0.1] bg-white/[0.09] text-white" : "text-white/34 hover:text-white/68"}`}>
              {icon}
        ...
        {previewMode !== "code" && (
          <div className="editor-panel-soft flex items-center gap-1 rounded-[18px] p-1">
            {([
              ...
              <button key={d} onClick={() => setDevicePreview(d)} title={label}
                className={`relative flex h-7 w-7 items-center justify-center rounded-xl transition-colors ${
                  devicePreview === d ? "border border-white/[0.1] bg-white/[0.09] text-white" : "text-white/34 hover:text-white/68"
                }`}>
                {icon}
                {selectedNode && d === "tablet" && selectedNode.responsiveHasTabletOverrides && (
                  <span className="absolute -top-0.5 -right-0.5 h-2 w-2 rounded-full bg-sky-300 shadow-[0_0_0_2px_rgba(8,8,9,0.9)]" />
                )}
          ...
          <div className="editor-ghost-divider h-7 w-px" />

        {previewMode !== "code" && (
          <button
            onClick={() => setVisualEditMode(!visualEditMode)}
            title={visualEditMode ? "Exit live edit (E)" : "Edit content directly (E)"}
            className={`flex h-8 items-center gap-1.5 rounded-[16px] px-3 text-[11px] font-medium border transition-all ${
              visualEditMode
                ? "border-amber-300/20 bg-[linear-gradient(180deg,rgba(245,158,11,0.26),rgba(234,88,12,0.12))] text-amber-50 shadow-[0_12px_24px_rgba(217,119,6,0.14)]"
                : "editor-action-btn"
            }`}>
            <MousePointer2 size={11} />
            {visualEditMode ? "Editing" : "Edit"}
            <kbd className="hidden lg:inline ml-0.5 text-[8px] opacity-40 font-mono">E</kbd>
          </button>
        )}
        ...
        {visualEditMode && previewMode === "preview" && (
          <button
            ...
            className={`flex h-8 items-center gap-1.5 rounded-[16px] px-3 text-[11px] font-medium border transition-all ${
              liveIsAI
                ? "border-teal-300/20 bg-[linear-gradient(180deg,rgba(45,212,191,0.22),rgba(20,184,166,0.08))] text-teal-50"
                : "editor-action-btn"
            }`}>
            {liveLoading ? <Loader2 size={11} className="animate-spin" /> : <Sparkles size={11} />}
            <span className="hidden sm:inline">Analyze</span>
            <kbd className="hidden lg:inline ml-0.5 text-[8px] opacity-40 font-mono">I</kbd>
          </button>
        )}

        {visualEditMode && breadcrumb.length > 0 && (
          <div className="editor-panel-soft ml-1 flex min-w-0 items-center gap-1 overflow-hidden rounded-[18px] px-3 py-1.5">
        ...
        <div className="flex-1" />
          ...
          <div className="editor-panel-soft flex items-center gap-1 rounded-[18px] p-1">
            <button onClick={handleUndo} disabled={!canUndo} title="Undo (⌘Z)"
              className="editor-action-btn flex h-7 w-7 items-center justify-center rounded-xl disabled:opacity-20 disabled:cursor-not-allowed">
              <Undo2 size={12} />
            </button>
        ...
        {previewMode !== "code" && (<>
          <div className="editor-panel-soft flex items-center gap-1 rounded-[18px] p-1">
            <button onClick={() => setZoom((z) => Math.max(z - 10, 40))} disabled={zoom <= 40} title="Zoom out"
              className="editor-action-btn flex h-7 w-7 items-center justify-center rounded-xl disabled:opacity-20 transition-colors">
              <ZoomOut size={12} />
            </button>
            <span className="w-10 text-center font-mono text-[10px] tabular-nums text-white/42">{zoom}%</span>
        ...
        {page?.status === "generating" && (
          <span className="editor-chip flex items-center gap-2 rounded-full px-3 py-1.5 text-[10px] text-teal-100">
      ...
      <div className="flex-1 overflow-hidden flex min-h-0">
        ...
        {(previewMode === "preview" || previewMode === "split") && (
          <div className={`overflow-auto flex-1 ${previewMode === "split" ? "border-r border-white/[0.06]" : ""}`}
            style={{ background: "radial-gradient(ellipse at 50% 0%, rgba(45,212,191,0.08) 0%, rgba(3,7,11,1) 72%)" }}>
            <div className="flex min-h-full items-start justify-center p-6 md:p-8">
              <div
                className={`editor-stage relative w-full max-w-[1380px] transition-[width] duration-300 ${
                  isConstrained
                ...
                <div className="editor-browser-chrome flex items-center justify-between gap-3 border-b border-white/[0.06] px-4 py-2">
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <div className="ml-2 min-w-0">
                      <p className="truncate text-[12px] font-medium text-white/82">{page?.name ?? "Preview"}</p>
                      <p className="truncate text-[10px] text-white/24">{visualEditMode ? "Live editor connected" : "Preview frame"}</p>
                    </div>
                  </div>
                  <span className="editor-chip rounded-full px-2.5 py-1 text-[9px] font-semibold uppercase tracking-[0.14em]">
                    {devicePreview}
                  </span>
                </div>
                <iframe
                  ref={internalIframeRef}
                  data-sitezy-preview-frame="editor"
                  aria-label={`Preview frame for ${page?.name ?? "current page"}`}
                ...
                {visualEditMode && (
                  <div
                    ...
                    style={{ boxShadow: "inset 0 0 0 2px rgba(251,191,36,0.22), 0 0 0 1px rgba(255,255,255,0.04)" }}
                  />
                )}
        ...
        {(previewMode === "code" || previewMode === "split") && (
          <div className={`editor-code-pane flex flex-col overflow-hidden ${previewMode === "split" ? "w-[42%] flex-shrink-0" : "flex-1"}`}>
            <div className="flex h-11 items-center gap-2 border-b border-white/[0.05] bg-[linear-gradient(180deg,rgba(255,255,255,0.03),rgba(255,255,255,0.012))] px-4 flex-shrink-0">
              <span className="editor-chip rounded-full px-2.5 py-1 text-[9px] font-semibold uppercase tracking-[0.16em]">{file?.language ?? "html"}</span>
              <div className="flex-1" />
              <span className="text-[10px] text-white/18 font-mono">{localCode.length.toLocaleString()} chars</span>
            </div>
            <textarea
              value={localCode}
              onChange={(e) => {
              ...
              spellCheck={false}
              className="flex-1 resize-none bg-transparent p-4 font-mono text-[12px] leading-relaxed text-[#c0f7ee] focus:outline-none"
              style={{ tabSize: 2 }}
            />
          </div>
        )}
